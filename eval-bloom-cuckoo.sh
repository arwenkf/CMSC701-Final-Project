...
for f in "${FPR_VALUES[@]}"; do
    for ((i=1; i<=NUM_RUNS; i++)); do
        ...
        java -cp "lib/*:src" Runner ../data/GCA_000006945.2/21-mers-GCA_000006945.2.txt "$f" "$QUERY_6945" "res-GCA_000006945-$f-$i.txt" "$2"
    done
done
...
for f in "${FPR_VALUES[@]}"; do
    for ((i=1; i<=NUM_RUNS; i++)); do
        ...
        java -cp "lib/*:src" Runner ../data/data_2/input.txt "$f" $QUERY_DATA2 "res-data2-$f-$i.txt" "$2"
    done
done
