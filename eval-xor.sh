...
for f in "${FPR_VALUES[@]}"; do
    ...
    for ((i=1; i<=NUM_RUNS; i++)); do
        ...
        java -cp "lib/*:src" queryXor bin-000006945 $QUERY_6945 "res-GCA_000006945-$f-$i.txt" $f
    done
done
...
for f in "${FPR_VALUES[@]}"; do
    for ((i=1; i<=NUM_RUNS; i++)); do
        ...
        java -cp "lib/*:src" queryXor bin-data2 $QUERY_DATA2 "res-data2-$f-$i.txt" $f
    done
done
